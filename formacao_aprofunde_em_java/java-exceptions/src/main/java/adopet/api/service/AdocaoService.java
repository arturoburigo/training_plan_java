...
import adopet.api.excepctions.AdocaoException;
...
public class AdocaoService {
    ...
    public void solicitar(SolicitacaoDeAdocaoDTO dto){
        ...
        if (pet.getAdotado()) {
            throw new AdocaoException("pet already adopted");
        }

        Boolean petAdocaoEmAndamento = adocaoRepository.existisByPetIdAndStatus(dto.idPet(), StatusAdocao.AGUARDANDO_AVALIACAO);

        if (petAdocaoEmAndamento) {
            throw new AdocaoException("pet already has adocao in progress");
        }

        Integer tutorAdocoes = adocaoRepository.countByTutorIdAndStatus(dto.idTutor(), StatusAdocao.APROVADO);

        if (tutorAdocoes >= 3) {
            throw new AdocaoException("tutor already has 2 adocao approved");
        }
